 ...
 * Misc fixes Copyright 2012 Eduardo José Tagle <ejtagle@tutopia.com>
 *
...
static const u16 rt5631_reg_default[RT5631_VENDOR_ID2 + 1] = {
#if 0
	[RT5631_SPK_OUT_VOL] = 0xc0c0,
...
#else
	[RT5631_SPK_OUT_VOL			] = 0x4949,		//speaker channel volume select SPKMIXER, 1.5dB by default
	[RT5631_HP_OUT_VOL			] = 0xc2c2,		//Headphone channel volume select OUTMIXER,- 7.5dB by default
	[RT5631_MONO_AXO_1_2_VOL	] = 0xf0c0,		//AXO1/AXO2 channel volume select OUTMIXER, -13.5dB by default
	[RT5631_ADC_REC_MIXER		] = 0xb0f0,		//Record Mixer source from Mic1/Mic2 by default
	[RT5631_MIC_CTRL_2			] = 0x8800, 	//Mic1/Mic2 boost 44dB by default
	...
	[RT5631_GEN_PUR_CTRL_REG	] = 0x2e00,		//Speaker AMP ratio gain is 1.99X (5.99dB)
	...
	[RT5631_INT_ST_IRQ_CTRL_2	] = 0x4f1c,		//enable HP zero cross
	[RT5631_MIC_CTRL_1        	] = 0x8000,     //change Mic1 to differential mode,mic2 to single end mode
	[RT5631_INDEX_ADD        	] = 0x0056, 	//Class D IR1
	[RT5631_INDEX_DATA        	] = 0x302f, 	//Class OC setting
	[RT5631_ALC_CTRL_1        	] = 0x0206, 	//ALC Attack time  = 170.667ms, Recovery time = 83.333 us
	[RT5631_ALC_CTRL_3        	] = 0x2000, 	//Enable for DAC path, Limit level = -6dB,
	...
	[RT5631_SDP_CTRL			] = 0x8000,
	[RT5631_MONO_SDP_CTRL		] = 0x8000,
	[RT5631_STEREO_AD_DA_CLK_CTRL] = 0x0010,
	[RT5631_MISC_CTRL			] = 0x2040,
	[RT5631_SOFT_VOL_CTRL		] = 0x07e0,
	[RT5631_PSEUDO_SPATL_CTRL	] = 0x0553,
#endif
...
static const HW_EQ_PRESET HwEq_Preset[] = {
	...
	{TF201_PAD 	,{0x0264,0xFE43,0xC111,0x1EF8,0x1D18,0xC1EC,0x1E61,0xFA19,0xC6B1,0x1B54,0x0FEC,0x0D41,0x095B,0xC0B6,0x1F4C,0x1FA5},0x403F, 0x8003, 0x0004},
	{TF201_DOCK ,{0x0264,0xFE43,0xC111,0x1EF8,0x1D18,0xC0CD,0x1F56,0x0FEC,0xC32B,0x1E5E,0xF805,0x1561,0x095B,0xC0B6,0x1F4C,0x1FA5},0x403F, 0x8003, 0x0004},
};  
...
static void rt5631_write_index_mask(struct snd_soc_codec *codec, unsigned int reg,unsigned int value,unsigned int mask)
{
	if (!mask)
		return;
	...
	if (mask!=0xffff)
	{
		unsigned int CodecData = rt5631_read_index(codec,reg);
		CodecData &= ~mask;
...
static void rt5631_update_eqmode(struct snd_soc_codec *codec, int mode)
{
	...
	if( mode == NORMAL )
	{
		/* clear EQ parameter */
		for( i=0; i <= 0x0f; i++)
			rt5631_write_index(codec, i, HwEq_Preset[mode].EqValue[i]);

		snd_soc_update_bits(codec, RT5631_EQ_CTRL, 0x003f, 0x0000);	/* disable Hardware LP,BP1,BP2,BP3,HP1,HP2 block Control */
	...
	else
	{
		...
		snd_soc_write(codec, RT5631_EQ_CTRL,HwEq_Preset[mode].HwEQCtrl);
		...
		/* update EQ parameter */
		rt5631_write_index(codec, 0x11, HwEq_Preset[mode].EqInVol); /* set EQ input volume */
		rt5631_write_index(codec, 0x12, HwEq_Preset[mode].EqOutVol); /* set EQ output volume */
		snd_soc_update_bits(codec, RT5631_EQ_CTRL, 0x4000, 0x4000);
	}
} 
...
static int rt5631_eq_sel_put(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	dev_dbg(codec->dev,"rt5631_eq_sel_put = %ld\n",ucontrol->value.integer.value[0]);

	rt5631_update_eqmode(codec, ucontrol->value.enumerated.item[0]);
	rt5631->eq_setting = ucontrol->value.enumerated.item[0];
	
	return 0;
} 

static int rt5631_eq_sel_get(struct snd_kcontrol *kcontrol,	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5631_priv *rt5631 = snd_soc_codec_get_drvdata(codec);
	
	ucontrol->value.enumerated.item[0] = rt5631->eq_setting;
	return 0;
}
...
static const char *rt5631_eq_sel[] = {"normal", "club", "dance", "live",
                        "pop", "rock", "oppo", "treble","bass","pad","dock"};
static const SOC_ENUM_SINGLE_EXT_DECL( rt5631_eq_enum, rt5631_eq_sel);
...
static const struct snd_kcontrol_new rt5631_snd_controls[] = {
	...
	/* Equalizer */
	SOC_ENUM_EXT("EQ Mode", rt5631_eq_enum, 
	...
	if (rt5631->master) {
		...
		for (i = 0; i < ARRAY_SIZE(codec_slave_pll_div); i++)
			if (freq_in == codec_slave_pll_div[i].pll_in &&
				...
				dev_info(codec->dev, "change PLL in slave mode\n");
				snd_soc_write(codec, RT5631_PLL_CTRL,
...
static void rt5631_sync_cache(struct snd_soc_codec *codec)
{
	...
	for (i = 2 ; i < size ; i += step) {
		/* Skip non accessible registers */
		if (!rt5631_readable_register(codec,i))
			continue;
		data[0] = i;
		data[1] = cache[i] >> 8;
		data[2] = cache[i];
                        ...
                        enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		if (codec->dapm.bias_level == SND_SOC_BIAS_OFF) {
			...
			rt5631_update_eqmode(codec, rt5631->eq_setting);
...
#define RT5631_FORMATS   (SNDRV_PCM_FMTBIT_S16_LE | \
                        SNDRV_PCM_FMTBIT_S20_3LE | \
                        SNDRV_PCM_FMTBIT_S24_LE | \
                        SNDRV_PCM_FMTBIT_S8)

static const struct snd_soc_dai_ops rt5631_dai_ops = {
...
/* Fetch register values from the hw registers to our cache */
static void rt5631_fill_cache(struct snd_soc_codec *codec)
{
	unsigned int i, step = codec->driver->reg_cache_step, size = codec->driver->reg_cache_size;
	u16 *cache = codec->reg_cache;
...
#ifdef CONFIG_PM
static int rt5631_suspend(struct snd_soc_codec *codec, pm_message_t mesg)
{
	dev_dbg(codec->dev, "%s()\n", __FUNCTION__);
...
static int rt5631_probe(struct snd_soc_codec *codec)
{
	...
	dev_info(codec->dev, "RT5631 probe called\n");
	...
	dev_dbg(codec->dev, "RT5631 codec version: %d\n", rt5631->codec_version);
	
	/* Fill cache with the default register values after reset*/
	rt5631_fill_cache(codec);

	/* Modify the default values to properly config the CODEC */
	for (i = 2; i < ARRAY_SIZE(rt5631_reg_default); i++) {
		if (!rt5631_readable_register(codec,i))
			continue;
		snd_soc_write(codec,i,rt5631_reg_default[i]);
	}

	/* Init the powerup sequence */
	snd_soc_update_bits(codec, RT5631_PWR_MANAG_ADD3,
			RT5631_PWR_VREF | RT5631_PWR_MAIN_BIAS,
			RT5631_PWR_VREF | RT5631_PWR_MAIN_BIAS);
	msleep(110);
...
static struct snd_soc_codec_driver soc_codec_dev_rt5631 = {
	...
	.reg_cache_default = rt5631_reg_default,
