		               ...
		               u64 mbr_offset, u64 first_sector, u64 first_size)
{
	...
	while (1) {
		if (++loopct > 2) {
			pr_err("tegra_msdos_parse_extended: loopcnt>2. exit\n");
			return;
		}

		pr_debug("tegra_msdos_parse_extended: read part sector, start=%llu+%llu size=%llu\n",
				mbr_offset, this_sector, this_size);

		data = read_dev_sector(bdev, mbr_offset+this_sector, &sect);
		if (!data) {
			pr_err("tegra_msdos_parse_extended: read error. exit\n");
			return; 
		}

		if (!msdos_magic_present(data + 510)) {
			pr_debug("tegra_msdos_parse_extended: no msdos magic. exit\n");
			goto done;
		}
		...
		for (i=0; i<4; i++, p++) {
			...
			pr_debug("tegra_msdos_parse_extended: put_partition %d start=%llu+%llu size=%llu\n",
					state->next, mbr_offset, next, size);
			put_partition(state, state->next++, mbr_offset+next, size);
			loopct = 0;
		}
	
		pr_debug("tegra_msdos_parse_extended: done with this sector\n");

		p -= 4;
		for (i=0; i<4; i++, p++)
			if (NR_SECTS(p) && is_extended_partition(p)) {
				pr_debug("tegra_msdos_parse_extended: extended part slot %d\n", i+1);
				break;
			}
	...
	pr_debug("tegra_msdos_parse_extended: done\n");
	put_dev_sector(sect);
}
...
int tegra_msdos_parse(struct parsed_partitions *state, struct block_device *bdev, u64 mbr_offset)
{
	...
	pr_debug("tegra_msdos_parse: mbr_offset=%llu\n", mbr_offset);

	data = read_dev_sector(bdev, mbr_offset, &sect);
	if (!data) {
		pr_err("tegra_msdos_parse: read error. exit\n");
		return -1;
	}
	if (!msdos_magic_present(data + 510)) {
		pr_debug("tegra_msdos_parse: no msdos magic\n");
		put_dev_sector(sect);
		return 0;
	}
	...
	for (slot = 1; slot <= 4; slot++, p++) {
		if (p->boot_ind != 0 && p->boot_ind != 0x80) {  // 0x1be,0x1ce,0x1de,0x1fe
			pr_debug("tegra_msdos_parse: slot %d, boot_ind=0x%x. exit\n", slot, p->boot_ind);
			put_dev_sector(sect);
			return 0;
		}
	}
	...
	for (slot = 1 ; slot <= 4 ; slot++, p++) {
		...
		pr_debug("tegra_msdos_parse: slot %d, start=%llu size=%llu\n", slot, start, size);
		if (!size)
			continue;
		if (is_extended_partition(p)) {
			pr_debug("tegra_msdos_parse: slot %d extended partition\n", slot);
		...
		pr_debug("tegra_msdos_parse: put_partition\n");
		put_partition(state, state->next++, start+mbr_offset, size);
	}

	pr_debug("\n");
	pr_debug("tegra_msdos_parse: done\n");
...
parse_tegrapart(struct parsed_partitions *state)
{
		...
		case STATE_BLOCKSIZE:
			...
			if ((*ptr==',') || (*ptr=='\0')) {
				...
				if (!strcasecmp(name, "mbr")) {
					pr_debug("parse_tegrapart: mbr start=%llu\n", offset);
					return tegra_msdos_parse(state, state->bdev, offset);
				}

				pr_debug("parse_tegrapart: part #%d [%s] start=%llu size=%llu\n",
	...
	pr_debug("parse_tegrapart: done without mbr\n");
	return ret;
}
...
{
	pr_debug("tegrapart_partition: state->bdev->bd_disk->disk_name = %s\n", state->bdev->bd_disk->disk_name);

/*	if (strcmp(state->bdev->bd_disk->disk_name, "mmcblk1")) {
		pr_debug("tegrapart_partition: exit\n");
		return 0;
	}
*/
	if (state->parts[0].size) {
		pr_debug("tegrapart_partition: part[0].size = 0. exit\n");
		return 0;
	}
