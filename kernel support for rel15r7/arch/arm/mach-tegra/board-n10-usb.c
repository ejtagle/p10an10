/*
 * arch/arm/mach-tegra/board-n10-usb.c
 *
...
static struct platform_device *n10_usb_devices[] __initdata = {
	&tegra_udc_device, 		/* USB gadget */
	&tegra_ehci3_device,
};
#endif

int __init n10_usb_register_devices(void)
{
	int ret = 0;
	
	/* Enable USB host power: 1=ON, 0=OFF */
	gpio_request(N10_TG_MINI_USB_ON_N,"USB HOST POWER");
	
	/* 2.2 N10 does this to enable Host mode */
	gpio_direction_input(N10_TG_MINI_USB_ON_N);
	...
	ret = platform_add_devices(n10_usb_devices, ARRAY_SIZE(n10_usb_devices));
	if (ret)
		return ret;
