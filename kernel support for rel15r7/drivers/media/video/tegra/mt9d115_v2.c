...
#define REG_MT9D115_PLL_CONTROL				0x0014
...
#define SENSOR_WAIT_PLL_LOCK		2 /* special number to indicate waiting for PLL lock */
#define SENSOR_WAIT_STDBY_EXIT	3 /* special number to indicate waiting for standby exit */
#define SENSOR_WAIT_SEQUENCER	4 /* special number to indicate waiting for sequencer to enter a given mode */
...
/* init sensor */
static struct sensor_reg const init_tbl[] = {
	// reset the sensor
	{0x001A, 0x0003},				// MIPI and SOC reset
	{SENSOR_WAIT_MS, 10},
	{0x001A, 0x0000},
	{SENSOR_WAIT_MS, 50},
	{SENSOR_TABLE_END, 0x0000}

	// Select CSI output interface 
...
static int sensor_write_table(struct sensor_info *info, const struct sensor_reg table[])
{
	...
	for (next = table; next->addr != SENSOR_TABLE_END; next++) {
		...
		if (next->addr == SENSOR_WAIT_PLL_LOCK) {
			u16 ctrl;
			int retry = 0;
			// poll & wait for pll to lock (PLL_LOCK set)
			do {
				if (retry++ > 100) {
					dev_err(&client->dev,"Unable to make mt9d115 sensor lock PLL (0x%x)", ctrl);
					return -ENODEV;
				}
				msleep(10);
				sensor_read_reg(info, REG_MT9D115_PLL_CONTROL, &ctrl);
		...
		if (next->addr == SENSOR_WAIT_STDBY_EXIT) {
			...
			do {
				if (retry++ > 10) {
					dev_err(&client->dev,"Unable to make mt9d115 sensor exit standby (0x%x)", ctrl);
					return -ENODEV;
				}
				msleep(100);
		...
		if (next->addr == SENSOR_WAIT_SEQUENCER) {
			...
			do {
				if (retry++ > 10) {
					dev_err(&client->dev,"Unable to make mt9d115 sequencer enter mode %d (current mode is %d)\n", mode, ctrl);
					return -EIO;
				}

				msleep(100);
		
				sensor_read_xdma(info, XDMA_SEQUENCER_ID, 4, &ctrl, 1, LENGTH_8_BIT);
...
static int sensor_set_exposure_brightness(struct sensor_info *info, int8_t exposure, int8_t brightness)
{
	...
	err = sensor_write_table(info, brightness_exposure_tbl[exposure][brightness]); 
...
static int sensor_set_wb(struct sensor_info *info, int8_t whitebalance)
{
	...
	switch(whitebalance) {
		case CAMERA_WB_MODE_AWB:
			err = sensor_write_table(info, wb_auto_tbl);
			break;

		case CAMERA_WB_MODE_INCANDESCENT:
			err = sensor_write_table(info, wb_incandescent_tbl);
			break;

		case CAMERA_WB_MODE_SUNLIGHT:
			err = sensor_write_table(info, wb_daylight_tbl);
			break;

		case CAMERA_WB_MODE_FLUORESCENT:
			err = sensor_write_table(info, wb_flourescant_tbl);
			break;

		case CAMERA_WB_MODE_CLOUDY:
			err = sensor_write_table(info, wb_cloudy_tbl);
			break;
...
static int sensor_set_contrast(struct sensor_info *info, int8_t contrast)
{
    ...
    switch (contrast)
    {
        case CAMERA_CONTRAST_0:
            err = sensor_write_table(info, contrast_tbl_0);
			break;

        case CAMERA_CONTRAST_1:
            err = sensor_write_table(info, contrast_tbl_1);
			break;

        case CAMERA_CONTRAST_2:
            err = sensor_write_table(info, contrast_tbl_2);
			break;
        
        case CAMERA_CONTRAST_3:
            err = sensor_write_table(info, contrast_tbl_3);
			break; 

        case CAMERA_CONTRAST_4:
            err = sensor_write_table(info, contrast_tbl_4);
			break;
...
static int sensor_set_saturation(struct sensor_info *info, int8_t saturation)
{
    ...
    switch (saturation)
    {
        case CAMERA_SATURATION_0:
            err = sensor_write_table(info, saturation_tbl_0);
			break;

        case CAMERA_SATURATION_1:
            err = sensor_write_table(info, saturation_tbl_1);
			break;

        case CAMERA_SATURATION_2:
            err = sensor_write_table(info, saturation_tbl_2);
			break;
        
        case CAMERA_SATURATION_3:
            err = sensor_write_table(info, saturation_tbl_3);
			break; 

        case CAMERA_SATURATION_4:
            err = sensor_write_table(info, saturation_tbl_4);
			break;
...
static int sensor_set_sharpness(struct sensor_info *info, int8_t sharpness)
{
    ...
    switch (sharpness)
    {
        case CAMERA_SHARPNESS_0:
            err = sensor_write_table(info, sharpness_tbl_0);
			break;

        case CAMERA_SHARPNESS_1:
            err = sensor_write_table(info, sharpness_tbl_1);
			break;

        case CAMERA_SHARPNESS_2:
            err = sensor_write_table(info, sharpness_tbl_2);
			break;
        
        case CAMERA_SHARPNESS_3:
            err = sensor_write_table(info, sharpness_tbl_3);
			break; 

        case CAMERA_SHARPNESS_4:
            err = sensor_write_table(info, sharpness_tbl_4);
			break;        
...
static int sensor_set_iso(struct sensor_info *info, int8_t iso_val)
{
    ...
    switch (iso_val)
    {
        case CAMERA_ISO_SET_AUTO:
            err = sensor_write_table(info, iso_tbl_auto);
			break;

        case CAMERA_ISO_SET_100:
            err = sensor_write_table(info, iso_tbl_100);
			break;

        case CAMERA_ISO_SET_200:
            err = sensor_write_table(info, iso_tbl_200);
			break;

        case CAMERA_ISO_SET_400:
            err = sensor_write_table(info, iso_tbl_400);
			break;
...
static int sensor_set_antibanding(struct sensor_info *info, int8_t antibanding)
{
    ...
    switch (antibanding)
    {
        ...
        case CAMERA_ANTIBANDING_SET_60HZ:
            err = sensor_write_table(info, banding_tbl_60);
			break;
		
        case CAMERA_ANTIBANDING_SET_50HZ:
            err = sensor_write_table(info, banding_tbl_50);
			break;

        case CAMERA_ANTIBANDING_SET_AUTO:
            err = sensor_write_table(info, banding_tbl_auto);
			break;
...
static int sensor_set_coloreffect(struct sensor_info *info, int8_t coloreffect)
{
	...
	switch(coloreffect) {
		case CAMERA_EFFECT_OFF:
			err = sensor_write_table(info, coloreffect_tbl_none);
			break;

		case CAMERA_EFFECT_MONO:
			err = sensor_write_table(info, coloreffect_tbl_mono);
			break;

		case CAMERA_EFFECT_SEPIA:
			err = sensor_write_table(info, coloreffect_tbl_sepia);
			break;

		case CAMERA_EFFECT_NEGATIVE:
			err = sensor_write_table(info, coloreffect_tbl_negative);
			break;

		case CAMERA_EFFECT_SOLARIZE:
			err = sensor_write_table(info, coloreffect_tbl_solarize);
			break;
...
static int sensor_set_sensor_mode(struct sensor_info *info, int mode)
{
    ...
    switch (mode)
    {
        case SENSOR_PREVIEW_MODE:
			err = sensor_write_table(info, mode_tbl_preview);
        ...
        case SENSOR_SNAPSHOT_MODE:
			err = sensor_write_table(info, mode_tbl_snapshot);
...
static int sensor_initialize(struct sensor_info *info)
{
    ...
    // Soft reset chip
    err = sensor_write_table(info, reset_tbl);
    if (err) {
		dev_err(&info->i2c_client->dev,"Unable to reset chip");
        goto quit;
    }
		
	// Select CSI as output
    err = sensor_write_table(info, csioutsel_tbl);
    if (err) {
		dev_err(&info->i2c_client->dev,"Unable to select CSI as output");
        goto quit;
    }
	
	
	model_id = 0x0000;
	err = sensor_read_reg(info, REG_MT9D115_MODEL_ID, &model_id);
    if (err) {
    ...
    err = sensor_write_table(info, pll_tbl);
    if (err) {
    ...
    model_id = 0x0000;
    err = sensor_read_reg(info, REG_MT9D115_MODEL_ID_SUB, &model_id);
    if (err) {
	...
	err = sensor_write_table(info, prev_snap_tbl);
    if (err) {
     ...
     * Exit Soft standby
     */
    err = sensor_write_reg(info, REG_MT9D115_STANDBY_CONTROL, 0x0028);
    if (err) {
	...
	// poll and wait for standby to exit (STANDBY_BIT cleared)
	sensor_read_reg(info, REG_MT9D115_STANDBY_CONTROL, &ctrl);
	dev_dbg(&info->i2c_client->dev, "standby reg: 0x%x", ctrl);
	
	do {
		if (retry++ > 10) {
			dev_err(&info->i2c_client->dev, "Unable to make mt9d115 sensor exit standby: got 0x%x", ctrl);
			err = -ENODEV;
			goto quit;
		}
		msleep(100);
     ...
     */
    err = sensor_write_reg(info, 0x098C, 0xA103);
    if (err) {
		dev_err(&info->i2c_client->dev,"failed to enable refresh mode [1]");
        goto quit;
    }
    err = sensor_write_reg(info, 0x0990, 0x0006);
    if (err) {
    ...
    err = sensor_write_reg(info,  0x0990, 0x0005);
    if (err) {
