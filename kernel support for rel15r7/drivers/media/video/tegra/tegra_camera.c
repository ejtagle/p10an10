...
void extern_tegra_camera_enable_vi(void)
{
	if(t20_dev) {
		/* turn on CSI regulator */
		tegra_camera_power_on(t20_dev);
		/* set EMC request */
		tegra_camera_enable_emc(t20_dev);
		/* enable camera HW clock */
		tegra_camera_enable_clk(t20_dev);
	}
}
...
void extern_tegra_camera_disable_vi(void)
{
	...
	if(t20_dev) {
		/* disable HW clock */
		tegra_camera_disable_clk(t20_dev);
		/* nullify EMC request */
		tegra_camera_disable_emc(t20_dev);
		/* turn off CSI regulator */
		tegra_camera_power_off(t20_dev);
	}
}
