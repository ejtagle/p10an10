...
status_t CameraHardware::setContrast(const V4L2CameraParameters& params)
{
    ...
    const char *str = params.get(V4L2CameraParameters::KEY_SCENE_MODE);
    int32_t value = attr_lookup(scenemode, sizeof(scenemode) / sizeof(str_map), str);

    if(value == CAMERA_BESTSHOT_OFF) {
        int contrast = params.getInt(V4L2CameraParameters::KEY_CONTRAST);
        if((contrast < CAMERA_MIN_CONTRAST)
                || (contrast > CAMERA_MAX_CONTRAST))
            return UNKNOWN_ERROR;

        ALOGV("setting contrast %d", contrast);
        mParameters.set(V4L2CameraParameters::KEY_CONTRAST, contrast);
...
status_t CameraHardware::setSaturation(const V4L2CameraParameters& params)
{
    ...
    int result;
    int saturation = params.getInt(V4L2CameraParameters::KEY_SATURATION);
    ...
    mParameters.set(V4L2CameraParameters::KEY_SATURATION, saturation);
...
status_t CameraHardware::setBrightness(const V4L2CameraParameters& params) {
...
status_t CameraHardware::setWhiteBalance(const V4L2CameraParameters& params)
{
    ...
    const char *str = params.get(V4L2CameraParameters::KEY_WHITE_BALANCE);
    if (str != NULL) {
        int32_t value = attr_lookup(whitebalance, sizeof(whitebalance) / sizeof(str_map), str);
        if (value != NOT_FOUND) {
            mParameters.set(V4L2CameraParameters::KEY_WHITE_BALANCE, str);
...
status_t CameraHardware::setZoom(const V4L2CameraParameters& params)
{
...
status_t CameraHardware::setAntibanding(const V4L2CameraParameters& params)
{
    int result;
    ...
    const char *str = params.get(V4L2CameraParameters::KEY_ANTIBANDING);
    if (str != NULL) {
        ...
        if (value != NOT_FOUND) {
            ...
            mParameters.set(V4L2CameraParameters::KEY_ANTIBANDING, str);
...
status_t CameraHardware::setFocusMode(const V4L2CameraParameters& params)
{
    const char *str = params.get(V4L2CameraParameters::KEY_FOCUS_MODE);
    if (str != NULL) {
      ALOGI("FocusMode =%s", str);
        int32_t value = attr_lookup(focus_modes,
                                    sizeof(focus_modes) / sizeof(str_map), str);
        if (value != NOT_FOUND) {
            mParameters.set(V4L2CameraParameters::KEY_FOCUS_MODE, str);
            ...
            if(mHasAutoFocusSupport){
                ...
                if(!strcmp(str, V4L2CameraParameters::FOCUS_MODE_CONTINUOUS_VIDEO) ||
                   !strcmp(str, V4L2CameraParameters::FOCUS_MODE_CONTINUOUS_PICTURE)){
                    cafSupport = TRUE;
                }
...
void CameraHardware::initDefaultParameters()
{
	...
	// white balance
	p.set(V4L2CameraParameters::KEY_SUPPORTED_WHITE_BALANCE,
		"auto,off,daylight,incandescent,fluorescent,cloudy_daylight");
	p.set(V4L2CameraParameters::KEY_WHITE_BALANCE,
		V4L2CameraParameters::WHITE_BALANCE_AUTO);

 	// antibanding
	p.set(V4L2CameraParameters::KEY_SUPPORTED_ANTIBANDING,
                    "auto,50hz,60hz,off");
	p.set(V4L2CameraParameters::KEY_ANTIBANDING, 
		V4L2CameraParameters::ANTIBANDING_AUTO);  
  	
	p.set(V4L2CameraParameters::KEY_SHARPNESS,
				CAMERA_DEF_SHARPNESS);
	p.set(V4L2CameraParameters::KEY_MAX_SHARPNESS,
		CAMERA_MAX_SHARPNESS);
		
    p.set(V4L2CameraParameters::KEY_CONTRAST,
                    CAMERA_DEF_CONTRAST);
    p.set(V4L2CameraParameters::KEY_MAX_CONTRAST,
            CAMERA_MAX_CONTRAST);

	p.set(V4L2CameraParameters::KEY_SATURATION,
                    CAMERA_DEF_SATURATION);	
    p.set(V4L2CameraParameters::KEY_MAX_SATURATION,
            CAMERA_MAX_SATURATION);
 
 	p.set("luma-adaptation", brightness);
	p.set("zoom", zoom_level);
