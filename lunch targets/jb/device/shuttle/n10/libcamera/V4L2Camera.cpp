...
#define CAMERA_POWER "/sys/devices/platform/n10-pm-camera/power_on"
...
int V4L2Camera::v4l2_s_ctrl( int fd,  int id, int value)
{
    int rc = 0;
    ...
    if(rc) {
        ALOGD("%s: fd=%d, S_CTRL, id=0x%x, value = 0x%x, rc = %ld\n",
                 __func__, fd, id, (uint32_t)value, rc);
   }
    return rc;
}

int V4L2Camera::v4l2_g_ctrl( int fd, int id, int *value)
{
    int rc = 0;
  struct v4l2_control control;
    ...
    if(rc) {
        ALOGD("%s: fd=%d, G_CTRL, id=0x%x, rc = %d\n", __func__, fd, id, rc);
     }
    *value = control.value;
    return rc;
}
...
int V4L2Camera::v4l2_query_ctrl( int fd, int id, int *min, int* max)
{
    int rc = 0;
    ...
    if(rc) {
        ALOGD("%s: fd=%d, QUERYCTRL, id=0x%x, rc = %d\n", __func__, fd, id, rc);
    }else {
...
int V4L2Camera::v4l2_set_antibanding (int fd, int antibanding) 
{
    int rc = 0;
...
int V4L2Camera::v4l2_set_whitebalance (int fd, WhiteBalance mode) 
{
    int rc = 0, value;
    int id;
    ...
    default:
        ...
        if(mode == wbDaylight) value = 6500;
        else if(mode == wbIncandescent) value = 2800;
        else if(mode == wbFluorescent ) value = 4200;
        else if(mode == wbCloudy) value = 7500;
        else
            value = 4200;
    }
    if(0 != (rc =  v4l2_s_ctrl(fd, id, value))){
        ALOGD("%s: error, setwb=%d, rc = %d\n", __func__, value, rc);
    }
    return rc;
}

int V4L2Camera::v4l2_get_whitebalance (int fd, WhiteBalance& mode) 
{
    int rc = 0, value;
    int id;

	if(0 != (rc =  v4l2_g_ctrl(fd, V4L2_CID_AUTO_WHITE_BALANCE, &value))){
        ALOGD("%s: error, getwb=%d, rc = %d\n", __func__, value, rc);
		return rc;
    }
	...
	if(0 != (rc =  v4l2_g_ctrl(fd, V4L2_CID_WHITE_BALANCE_TEMPERATURE, &value))){
        ALOGD("%s: error, getwb=%d, rc = %d\n", __func__, value, rc);
		return rc;
    }
...
bool V4L2Camera::QueryCtl(VideoCtl ctl,CtlInfo& info)
{
    switch(ctl)  {
    case ctlSharpness:
        return v4l2_query_ctrl(fd, V4L2_CID_SHARPNESS, &info.min, &info.max) == 0;
    case ctlContrast:
        return v4l2_query_ctrl(fd, V4L2_CID_CONTRAST, &info.min, &info.max) == 0;
    case ctlSaturation:
        return v4l2_query_ctrl(fd, V4L2_CID_SATURATION, &info.min, &info.max) == 0;
    case ctlBrightness:
        return v4l2_query_ctrl(fd, V4L2_CID_BRIGHTNESS, &info.min, &info.max) == 0;
    case ctlWhiteBalance:
        return v4l2_query_ctrl(fd, V4L2_CID_AUTO_WHITE_BALANCE, &info.min, &info.max) == 0;
    case ctlZoom:
        return v4l2_query_ctrl(fd, V4L2_CID_ZOOM_ABSOLUTE, &info.min, &info.max) == 0;
    case ctlAntibanding:
        return v4l2_query_ctrl(fd, V4L2_CID_POWER_LINE_FREQUENCY, &info.min, &info.max) == 0;
    default:
        ALOGE("%s: default: control %d not supported\n", __func__, ctl);
        break;
    }
    return false;
}

bool V4L2Camera::GetCtl(VideoCtl ctl,int& val)
{
    switch(ctl)  {
    case ctlSharpness:
        return v4l2_g_ctrl(fd, V4L2_CID_SHARPNESS, &val) == 0;
    case ctlContrast:
        return v4l2_g_ctrl(fd, V4L2_CID_CONTRAST, &val) == 0;
    case ctlSaturation:
        return v4l2_g_ctrl(fd, V4L2_CID_SATURATION, &val) == 0;
    case ctlBrightness:
        return v4l2_g_ctrl(fd, V4L2_CID_BRIGHTNESS, &val) == 0;
    case ctlWhiteBalance:
        return v4l2_get_whitebalance(fd, (WhiteBalance&) (&val)) == 0;
    case ctlZoom:
        return v4l2_g_ctrl(fd, V4L2_CID_ZOOM_ABSOLUTE, &val) == 0;
    case ctlAntibanding:
        return v4l2_g_ctrl(fd, V4L2_CID_POWER_LINE_FREQUENCY, &val) == 0;
    default:
        ALOGE("%s: default: control %d not supported\n", __func__, ctl);
        break;
    }
...
bool V4L2Camera::SetCtl(VideoCtl ctl,int val)
{
    switch(ctl)  {
    case ctlSharpness:
        return v4l2_s_ctrl(fd, V4L2_CID_SHARPNESS, val) == 0;
    case ctlContrast:
        return v4l2_s_ctrl(fd, V4L2_CID_CONTRAST, val) == 0;
    case ctlSaturation:
        return v4l2_s_ctrl(fd, V4L2_CID_SATURATION, val) == 0;
    case ctlBrightness:
        return v4l2_s_ctrl(fd, V4L2_CID_BRIGHTNESS, val) == 0;
    case ctlWhiteBalance:
        return v4l2_set_whitebalance (fd, val) == 0;
    case ctlZoom:
        return v4l2_s_ctrl(fd, V4L2_CID_ZOOM_ABSOLUTE, val) == 0;
    case ctlAntibanding:
        return v4l2_set_antibanding (fd, val) == 0;
    default:
        ALOGE("%s: default: control %d not supported\n", __func__, ctl);
        break;
    }
