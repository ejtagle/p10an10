...
#define ING_SERVICE  "ing"
static int ing_start(void)
{
  property_set("ctl.start", ING_SERVICE);
  return wait_for_property("init.svc." ING_SERVICE, "running", 10);
}

static int is_ing_running(void)
{
  char value[PROPERTY_VALUE_MAX] = {'\0'};
  property_get("init.svc." ING_SERVICE, value, NULL);
  return strcmp(value, "running") == 0;
}

static int ing_stop(void)
{
  property_set("ctl.stop", ING_SERVICE);
  return wait_for_property("init.svc." ING_SERVICE, "stopped", 10);
}

static const char* ingWakeLock = "gpsWakeLock";
...
static int open_gps()
{
	...
	// Start the ing Service if not running
	if (!is_ing_running()) {
		ing_start();
	}
	
    // look for a kernel-provided gps_data port
    if (property_get("ro.kernel.gps.data",prop_value,"/dev/ttyp0") == 0) {
	...
	// Acquire a partial wakelock so we can track satellites
	acquire_wake_lock(PARTIAL_WAKE_LOCK, ingWakeLock);
	
	D("gps will read from %s", prop_value);

	// Turn on GPS function
	serial_write(gps_fd,"$PUNV,WAKEUP*2C\r\n");
	usleep(1000000);
...
static void close_gps(int gps_fd)
{
	...
	if ( gps_fd >= 0 ) {
		...
		serial_write(gps_fd,"$PUNV,SLEEP*7E\r\n");
		usleep(1000000);

		// Reduce NMEA message rates
		close( gps_fd );
		
		// Release wakelock
		release_wake_lock(ingWakeLock);
	}
	
	// Stop the ing Service if running
	if (is_ing_running()) {
		ing_stop();
	}
...
static int gps_inject_time(GpsUtcTime time_ms, int64_t timeReference, int uncertainty)
{
#if 0
	char buf[64];
	// time_ms = ms from jan 1, 1970 
	struct tm utc;
	unsigned int ms = time_ms % 1000; 	// Milliseconds
	time_t time_s = time_ms / 1000;		// Seconds
	
	// Convert to UTC */
	gmtime_r( time_s, &utc );
	
	sprintf(buf,"$PUNV,ASSIST,%02d%02d%02d.%02d,%02d%02d%02d",
		utc.tm_hour,utc.tm_min,utc.tm_sec,ms/10,utc.tm_mday,utc.tm_mon+1,utc.tm_year % 100);
