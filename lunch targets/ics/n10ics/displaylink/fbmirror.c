...
int open_fb(fb_t* ctx,int idx)
{
	...
	/* Print some screen info currently available */
  	LOGD("Screen resolution: (%dx%d)\n", vinfo.xres,vinfo.yres);
  	LOGD("Line width in bytes %d\n", finfo.line_length);
  	LOGD("bits per pixel : %d\n", vinfo.bits_per_pixel);
  	LOGD("Red: length %d bits, offset %d\n", vinfo.red.length, vinfo.red.offset);
  	LOGD("Green: length %d bits, offset %d\n", vinfo.green.length, vinfo.green.offset);
  	LOGD("Blue: length %d bits, offset %d\n", vinfo.blue.length, vinfo.blue.offset);
	...
	LOGI("The framebuffer '%s' was mapped to memory successfully at %p.\n",buf,ctx->pixels);
	return 1;
}
...
int close_fb(fb_t* ctx)
{
	...
	LOGI("Framebuffer closed successfully\n");
	return 1;
}
...
int transfer_fb(fb_t* src,fb_t* dst)
{
	...
	LOGE("Unsupported transfer:  src bpp: %d, dst bpp: %d\n",src->bpp,dst->bpp);
	return 0;
}
...
{
	fb_t src,dst;
	LOGI("Starting DisplayLink mirroring service...\n");
	
waitit:
	/* Wait until a DisplayLink framebuffer appears */
	
	LOGD("Trying to open a DisplayLink framebuffer\n");
	while (!open_fb(&dst,2)) {
		sleep(1);
	};

	LOGI("DisplayLink adapter plugged in!\n");

	LOGD("Trying to open main framebuffer\n");
	if (!open_fb(&src,0)) {
		LOGE("Failed to open main framebuffer\n");
		close_fb(&dst);
		goto waitit;
	}
	
	LOGI("Main framebuffer opened -- Starting mirroring operation\n");
	while(1) {
		/* Copy from one to the other */
		if (!transfer_fb(&src,&dst)) {
			LOGE("Failed to transfer... Assume DuisplayLink adapter was removed\n");
			close_fb(&dst);
			close_fb(&src);
			goto waitit;
		}
